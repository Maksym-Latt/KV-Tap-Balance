...
@Singleton
class GameRepository @Inject constructor(private val preferences: PreferenceDataSource) {
    val bestScore: Flow<Long> = preferences.bestScoreFlow
    val points: Flow<Int> = preferences.pointsFlow
    val selectedSkin: Flow<ChickenSkin> =
            preferences.selectedSkinFlow.map { id ->
    ...
    val skinsWithUnlockState: Flow<List<ChickenSkinState>> =
            combine(unlockedSkins, selectedSkin) { unlocked, selected ->
                ChickenSkin.entries.map { skin ->
                    ChickenSkinState(
                            skin = skin,
                            isUnlocked = unlocked.contains(skin.id) || skin == ChickenSkin.WhiteHen,
